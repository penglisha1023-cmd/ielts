...
(function () {
  ...
  function pushScore() {
    ...
    IELTS.submitScore(TEST_ID, correct, qs.length, timerSecs(), { perQ }).catch(console.error);
  }

  function bindFinish() {
    const btn = document.querySelector('#finish');
    if (!btn || btn.dataset.bridged) return;
    btn.dataset.bridged = '1';
    btn.addEventListener('click', () => {
      // Grading runs synchronously inside the template's own click handler;
      // wait a tick, then submit only when we've entered review mode.
      setTimeout(() => {
        const reviewing = !!(window.App && App.state && App.state.isReviewing);
        if (reviewing) pushScore();
      }, 120);
    });
  }
